# PowerShell script to seed data into Cosmos DB and Azure AI Search
Write-Host "🚀 Starting data seeding..."

# Check if .env file exists
$envFile = "../.env"
if (-not (Test-Path $envFile)) {
    Write-Host "❌ .env file not found. Please run get-keys.ps1 first."
    exit 1
}
...
pip install azure-cosmos azure-search-documents requests --quiet
...
def setup_search_service():
    ...
    # Use SEARCH_SERVICE_KEY instead of SEARCH_ADMIN_KEY
...
def main():
    ...
    required_vars = ['COSMOS_ENDPOINT', 'COSMOS_KEY', 'SEARCH_SERVICE_ENDPOINT', 'SEARCH_SERVICE_KEY']
...
$pythonScript | Out-File -FilePath "seed_data.py" -Encoding utf8

# Run the Python script
Write-Host "🐍 Running data seeding script..."
python seed_data.py

# Clean up
Remove-Item "seed_data.py" -ErrorAction SilentlyContinue
